SELECT  Flights.FlightId,
		...
		(Flights.EngineEndOperatingCounterInSeconds - Flights.EngineStartOperatingCounterInSeconds) AS EngineDurationInSec,
        ...
        WHERE      (FlightCrew.FlightId = Flights.FlightId) AND (FlightCrew.FlightCrewType = 1) AND FlightCrew.IsDeleted = 0) AS GliderPilot,
		...
		WHERE F.FlightId = Flights.FlightId AND (FlightCrew.FlightCrewType = 1) AND FlightCrew.IsDeleted = 0 and PersonClub.ClubId = FlightTypes.ClubId AND PersonClub.IsDeleted = 0) AS PilotsMemberNumber,
        ...
        WHERE      (FlightCrew_4.FlightId = Flights.FlightId) AND (FlightCrew_4.FlightCrewType = 2) AND FlightCrew_4.IsDeleted = 0) AS GliderCoPilot,
        ...
        WHERE      (FlightCrew_3.FlightId = Flights.FlightId) AND (FlightCrew_3.FlightCrewType = 3) AND FlightCrew_3.IsDeleted = 0) AS GliderInstructor,
		
		(SELECT     CONCAT(Persons_3.Lastname, ' ', Persons_3.Firstname) AS GliderPAX
        FROM          FlightCrew AS FlightCrew_3 INNER JOIN
                                Persons AS Persons_3 ON FlightCrew_3.PersonId = Persons_3.PersonId
        WHERE      (FlightCrew_3.FlightId = Flights.FlightId) AND (FlightCrew_3.FlightCrewType = 4) AND FlightCrew_3.IsDeleted = 0) AS GliderPAX,
	
		(SELECT     CONCAT(Persons_3.Lastname, ' ', Persons_3.Firstname) AS InvoiceRecipient
        FROM          FlightCrew AS FlightCrew_3 INNER JOIN
                                Persons AS Persons_3 ON FlightCrew_3.PersonId = Persons_3.PersonId
        WHERE      (FlightCrew_3.FlightId = Flights.FlightId) AND (FlightCrew_3.FlightCrewType = 10) AND FlightCrew_3.IsDeleted = 0) AS InvoiceRecipient,
	...
	Flights.DeliveryCreatedOn,
	Flights.FlightReportSentOn,
	Flights.CreatedOn, 
    Flights.ModifiedOn,
	...
	(SELECT     CONCAT(Persons_3.Lastname, ' ', Persons_3.Firstname) AS TowPilot
        FROM          FlightCrew AS FlightCrew_3 
			INNER JOIN Persons AS Persons_3 ON FlightCrew_3.PersonId = Persons_3.PersonId
        WHERE      (FlightCrew_3.FlightId = Flights_1.FlightId) AND (FlightCrew_3.FlightCrewType = 1) AND FlightCrew_3.IsDeleted = 0) AS TowPilot,
	...
	(SELECT     CONCAT(Persons_3.Lastname, ' ', Persons_3.Firstname) AS WinchOperator
        FROM          FlightCrew AS FlightCrew_3 INNER JOIN
                                Persons AS Persons_3 ON FlightCrew_3.PersonId = Persons_3.PersonId
        WHERE      (FlightCrew_3.FlightId = Flights.FlightId) AND (FlightCrew_3.FlightCrewType = 5) AND FlightCrew_3.IsDeleted = 0) AS WinchOperator,
		Flights.AirStateId AS GliderFlightAirStateId,
		Flights_1.AirStateId AS TowFlightAirStateId,
		Flights.ProcessStateId AS GliderFlightProcessStateId,
		Flights_1.ProcessStateId AS TowFlightProcessStateId
FROM Flights 
