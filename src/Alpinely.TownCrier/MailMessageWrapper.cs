...
namespace Alpinely.TownCrier
{
    public class MailMessageWrapper
    {
        ...
        protected internal IDictionary<string, object> TokenValues;
        ...
        public MailMessageWrapper WithHtmlBodyFromVelocity(string bodyTemplate)
        {
            if (HtmlBody != null)
                throw new InvalidOperationException("An HTML body already exists");
        ...
        public MailMessageWrapper WithHtmlBodyFromVelocityFile(string filename)
        {
            return WithHtmlBodyFromVelocity(File.ReadAllText(filename));
        }
